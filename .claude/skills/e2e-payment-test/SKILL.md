...
1. `browser_navigate` → `http://localhost:3001/this-page-does-not-exist`
2. `browser_snapshot`：確認頁面內容與 Nav 結構
3. ✅ 預期（共 4 項）：
   - 頁面標題包含「找不到頁面」
   - **無標準 Nav**，只有品牌「花漾生活」與「← 回到首頁」（設計稿專用 Nav）
   - 顯示「PAGE NOT FOUND」eyebrow、「404」大字、「找不到頁面」標題
   - 底部有「花漾生活 — 每一束花，都值得被找到」品牌文字
4. `browser_click`：點選「回到首頁」按鈕
5. ✅ 預期：跳轉至 `http://localhost:3001/`，頁面標題包含「首頁」
